...
**路径**：从一个节点到另一个节点之间分支构成的路径
**路径长度**：分支数
**树的路径长度**：从根节点到每个节点的路径长度之和，记为 *TL*
**权**：分为节点权和边权
**节点的带权路径长度**：从根节点到改节点的路径长度乘以该节点的权
**树的带权路径长度**：从根节点到叶子节点的带权路径长度之和，记作 *WPL*
![[Pasted image 20260106082415.png]]

可见：满二叉树不一定是哈夫曼树，哈夫曼树中权越大的叶子离根越近，相同WPL的哈夫曼树不唯一

## 构造哈夫曼树

![[Pasted image 20260106082939.png]]

**可见**：有 *n* 个节点，需要经过 *(n-1)* 次合并，产生 *(n-1)* 个新节点，构成的哈夫曼树有 *(2n-1)* 个节点，每个节点的度为 0 或 2

```cpp
// 哈夫曼树，节点储存在数组中(len = 2n)(第一个空间不用)
typedef struct {
    int weight;
...
void creatHT(HT &tree, int n) {
    ...
    for (int i = n; i < m; i++) {
        int s1, s2;
        // 选取HT中1到i中（包括i） parent==0 的 weight 最小的两个节点，用s1/2储存它们的索引
        Select(tree, i, s1, s2);
        tree[s1].parent = tree[s2].parent = i+1;
        tree[i+1].lchild = s1;
        tree[i+1].rchild = s2;
        tree[i+1].weight = tree[s1].weight + tree[s1].weight
    }
}
```
![[Pasted image 20260106090643.png]]

WPL = SUM(9~15)

## 哈夫曼编码

不等长编码 要使任一字符编码都不是另一个字符编码的前缀

![[Pasted image 20260107142350.png]]

哈夫曼编码能保证前缀编码，因为每个字符都是叶子节点，前缀绝不会和其它字符的编码重合
哈夫曼编码能保证编码总长度最短，因为它的带权路径长度最小

```cpp
// 哈夫曼编码，给n个字符编码
void CreateHCode(HT tree, char **&code, int n) {
    code = new char*[n + 1];   // 首位置不用
    char *cd = new char[n];    // 编码长度最多为n-1，留一个位置存放'\0'
    int start = n - 1;
    cd[start] = '\0';
    for (int i = 1; i <= n; i++) {
        start = n - 1;
        int c = i, f;
        while ((f = tree[c].parent) != 0) {  // c不是根节点时
            cd[--start] = tree[f].lchild == c ? '0' : '1';
            c = f;
        }
...
![[Pasted image 20260107155457.png]]